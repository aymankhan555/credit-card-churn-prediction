...
if st.sidebar.button("Predict Churn Risk",type='primary'):
    ...
    with main_features_col:
        ...
        st.dataframe(input_df[main_features].T.rename_axis('Features').rename(columns={0:'Values'}))
    with engineered_features_col:
        st.markdown("**Engineered Features**")
        eng_fix = input_df_new[Engineered_features].round(2).astype(str).replace('0.0', '0')
        st.dataframe(eng_fix.T.rename_axis('Features').rename(columns={0:'Values'}))
